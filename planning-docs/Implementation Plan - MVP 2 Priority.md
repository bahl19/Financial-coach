...
#### Task 0. Unblock the repo for its first `mvp2/` package — **DONE (commit `852a48b`)**

Phase MVP2-0 deliberately left tripwires that fire the moment `mvp2/` appears. These were **not** optional cleanup: two turned the suite red on the very first commit of this cut, and one silently weakened CI. All were verified against the real repo rather than assumed, and all three are now fixed ahead of the session so implementation starts from a green, correctly-guarded repo:

- [x] **`test_no_mvp2_package_exists_yet` replaced, not deleted.** It asserted `not (REPO_ROOT / "mvp2").exists()`; its own docstring specified the correct resolution (*"replaced by real per-file graph enforcement using `_MVP2_DEPENDENCY_GRAPH`, not silently deleted"*). It is now `test_mvp2_files_respect_the_dependency_graph` (real per-file enforcement) plus `test_no_unknown_mvp2_subpackage_appears`, which preserves the original guard's spirit — an unplanned `mvp2/whatever/` still fails rather than silently inheriting no rules.
- [x] **Shared-leaf gap in `_MVP2_DEPENDENCY_GRAPH` closed.** It mapped `"mvp2.profile": {"mvp1_public"}`, so enforcing it literally made `mvp2/profile/*` importing `mvp2.contracts`/`errors`/`hashing` a violation — yet the plan's own package layout requires exactly those imports. Resolved with `_MVP2_SHARED_LEAVES = {"mvp2.contracts", "mvp2.errors", "mvp2.hashing"}`, universally importable by every subpackage, with the leaf rule itself still enforced (stdlib/typing/Pydantic only — never `utils`/`agents`/Streamlit/a sibling subpackage). No real arrow was loosened. `test_graph_checker_allows_the_planned_profile_layout` pins the exact case that was wrongly rejected, so the bug cannot silently return.
- [x] **CI mypy divergence removed permanently.** Rather than hardcoding `mvp2` (which breaks CI *now*, since the directory does not exist) or leaving the "remember to edit this" comment, `.github/workflows/ci.yml` now mirrors `scripts/verify_mvp2_phase.py`'s `_mypy_targets()` with a `if [ -d mvp2 ]` conditional. Correct in both states, no manual step to forget. Verified by running both branches locally.
- [x] **Relative-import bypass hole closed** (found while implementing, not in the original audit). The old `_imported_module_names` skipped relative imports entirely (`node.level == 0` guard), so `from ..strategy import x` would have dodged graph enforcement completely — making the check vacuous in exactly the way it exists to prevent. Relative imports are now resolved against the file's own package. `test_graph_checker_rejects_a_planted_relative_later_phase_import` pins it.

**Deliberately NOT done: `PHASE_CHECKS[1]` in `scripts/verify_mvp2_phase.py`.** Adding it now would make `--phase 1` print *"All checks passed through phase 1"* while zero Phase-1 code exists — actively misleading, and worse than the current honest `exit 2` ("no checks defined for phase 1"). That script's own comment already forbids guessing checks ahead of the phase landing. Add it in the same commit as the first `mvp2/profile/` code, not before.

**Verification:** 433 passed, 4 skipped (was 426 — 7 new boundary tests). `ruff` and `mypy` clean. `scripts/verify_mvp2_phase.py --phase 0` green. Enforcement was additionally proven end-to-end against a throwaway real `mvp2/` package — a legal layout passed, a forward-phase import failed, a relative-import version of the same violation failed, and an unapproved subpackage failed — then the scratch package was removed.
...
2. `PHASE_CHECKS[1]` is added to `scripts/verify_mvp2_phase.py` in the same commit as the first `mvp2/profile/` code, and `--phase 1` runs green. (Task 0's other items are already done — see above.)
